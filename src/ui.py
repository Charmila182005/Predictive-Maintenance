...
st.set_page_config(
    page_title="Machine Health | Maintenance Pilot",
    page_icon="⚙️",
    layout="wide",
)
...
try:
    ...
    st.error(
        f"Models could not be loaded: {exc}. "
        "Run training and verify the artifact manifest."
    )
    st.stop()
...
with st.sidebar:
    ...
    st.write(
        "Synthetic milling data. Risk applies to the supplied readings; "
        "there is no time-to-failure estimate."
    )
    st.write(
        "Factory calibration and human approval are required before "
...
if "result" in st.session_state:
    ...
    for action in result["recommended_maintenance_action"]:
        st.write("• " + action)
    ...
    with tabs[0]:
        st.caption(
            "Evidence-supported modes appear first. Probabilities, "
            "context indicators, and base rates have different meanings; "
            "scores are not directly comparable."
        )

        for mode in result["likely_failure_modes"]:
            st.write(
                f"**{mode.get('mode', mode.get('failure_mode', 'Unknown'))}**"
            )
        ...
        for evidence in result["condition_evidence"]:
            with st.expander(
                ...
                + (
                    ...
                    else "No condition triggered"
                )
            ):
                st.write(evidence["rule"])
                st.json(evidence["measured"])
    ...
    with tabs[1]:
        ...
        for feature in result["contributing_features"]:
            st.write(
                f"**{feature.get('feature', feature.get('name', 'Feature'))}**"
            )
    ...
    # ---------------------------------------------------------
    with tabs[2]:
        st.subheader("Inputs and derived values")
    ...
    with tabs[3]:
        st.caption(
            "This session only. These are prediction timestamps, "
            "not a run-to-failure dataset."
        )

        for item in st.session_state["history"]:
            st.write(
                f'**{item["prediction_timestamp"]}** · '
                f'Machine: {item["machine_id"]} · '
                f'Failure probability: '
                f'{item["failure_probability"]:.1%} · '
    ...
    st.info(
        "Enter all six values and select Assess machine condition. "
        "The example is ready to run."
    )