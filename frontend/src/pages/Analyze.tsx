...
import {
  StatusBadge,
  toneForStatus,
} from "@/components/instrument/StatusBadge";
  ...
  return (
    <div className="grid grid-cols-3 gap-3">
      {(
        ...
        <div
          key={sym}
          className="rounded-lg border border-hairline bg-surface/50 px-2.5 py-2.5"
        >
...
export function Analyze() {
  ...
  const [form, setForm] =
    useState<AssessInput>({
      ...DEFAULTS,
    });

  const [alertFiled, setAlertFiled] =
    useState(false);
      ...
      {!assessment && status === "idle" && (
        <div className="flex items-center gap-2 rounded-lg border border-hairline bg-overlay/50 px-4 py-2.5">
          ...
          <p className="text-[12px] text-ink-2">
            Live model first. If the shared backend
            is unreachable, results fall back to the{" "}
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {connectionMessage && (
        <div
          className={cn(
            "flex items-center gap-2 rounded-lg px-3.5 py-2.5",
            isLive
              ? "border border-status-healthy/30 bg-status-healthy/5"
              : "border border-status-elevated/30 bg-status-elevated/8"
          )}
        >
          <StatusDot
            tone={
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-[11.5px] text-ink-3">
          <span>
            Model may label output as{" "}
            {a.mode.code === "NONE"
              ? FAILURE_MODES.NONE
                  .name
              : FAILURE_MODES[
                  a.mode.code as keyof typeof FAILURE_MODES
                ]?.name}
            .
          </span>
        </div>
      ...
      <div className="flex justify-end">
        <Link
          to="/machines"
          className="flex items-center gap-1 font-mono text-[10px] uppercase tracking-[0.14em] text-ink-3 hover:text-signal"
        >
          View machine health
